/**
 * This file is for opening up e2o in the dev environment. It create a background process and launches electron in it.
 * This file came from the Finsemble-seed project
 */
...
module.exports = function (params, cb) {
	...
	logger.debug('e2oLauncher. Electron executable path:', execPath);
	const electronPath = execPath;
	let debugArg = '';
	debugArg = envOrArg('breakpointOnStart') === 'true' ? '--inspect-brk=5858' : '--inspect=5858';

	const indexPath = path.join(__dirname, './devIndex.js');
	...
	ON_DEATH((signal, err) => {
		if (electronProcess) {
			logger.log('FEA Shutdown. Killing electron processes.');
