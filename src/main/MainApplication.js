...
/**
 * All windows must be apart of an application and their references are stored here
 */
module.exports = class MainApplication extends EventEmitter {
	/**
	 *
	...
	constructor(params) {
		...
		this.appWindow = null;// The main application window. It is also stored in the child window list
		this.details = params;
		...
		// If the name isn't provided, default to the application's UUID. This mimics OF.
	...
	/**
	 * Creates the applications window. Each window has a main window and a list of child windows.
	 * @param {MainApplicationParams} params
	 * @param {Function} cb
	 *
	 * @return undefined
	...
	async start(params, cb = Function.prototype) {
		this.setupListeners();
		// console.log("Creating MainApplication window:", params);
		try {
			const win = await this.createWindow(params);
			this.processId = this.getProcessId();
			cb(null);
		} catch (err) {
			logger.error(`Failed to create the application window ${err.message}`);
			cb(err);
		}
	}
	 ...
	 */
	setupListeners() {
		PermissionsManager.addRestrictedListener(`${this.uuid}-spawn`, this.createWindowRequest, 'Application.spawn');
		PermissionsManager.addRestrictedListener(`Application-${this.uuid}-close`, this.closeRequest, 'Application.close');
		PermissionsManager.addRestrictedListener(`${this.uuid}-getManifest`, this.getManifest, 'Application.getManifest');
	...
	getHeuristics() {
		...
		return {
			...
			name: this.uuid, // the application name
			...
			uuid: this.uuid, // the application UUID
	...
	/**
	 * update the resources object on the application with new information.
	...
	/**
	 * When a window within an application closes the application is informed. If it's the last window in the application, the application closes itself.
	 *
	...
	windowClosed(windowName) {
		...
		if (this.appWindow && this.appWindow.windowName === windowName) delete this.appWindow;
	...
	/**
	 * This is called when a request comes in to close an application.
	...
	/**
	 * This closes all child windows and then closes the application.
	 *
	 * @return undefined
	 */
	close() {
		logger.info(`Closing application ${this.uuid}`);
		...
		logger.log(`APPLICATION LIFECYCLE: Application ${this.uuid} closed.`);
		this.emit('close');
	}

	/**
	 * This will recreate the main window if an application is restarted.
	 * //@todo Does this work?
	 */
	async finishRestart() {
	...
	/**
	 * This only works for the main application. It gets the startup manifest. That is how OpenFin does it.
	 * @param {BusEvent} eventObj
	 */
	...
	/**
	 * Handles a request to create a window for an application
	 * @param {BusEvent} eventObj
	 */
	async createWindowRequest(eventObj) {
		// wait for application to finish being created before creating the window.
	...
	async createWindow(params, cb = Function.prototype) {
		logger.debug(`MainApplication->CreateWindow. Creating window ${params.name}`);
		params.appUUID = this.uuid;

		// create a combined permission set for the new window using the applications permissions and the window's requested permissions, keeping the more restrictive ones
		...
		// Domain finsemble is running in
		params.finsembleDomain = this.finsembleDomain;
		...
		try {
			...
			// ApplicationManager.findWindowByName loops through applications and looks at the appWindow property.
			...
			logger.verbose(`Application ${this.details.name} initializing ${mainWindow.windowName}.`);
			await mainWindow.init();
			logger.verbose(`Application ${this.details.name} initialized ${mainWindow.windowName}.`);
			// @todo refactor: No anonymous handlers.
			mainWindow.addListener('close', () => {
				logger.debug(`Application ${this.details.name} received close event for ${mainWindow.windowName}`);
				const closeWindow = this.windowClosed.bind(this);
				closeWindow(mainWindow.windowName);
			});
