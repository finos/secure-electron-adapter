...
describe('PermissionsManager class', () => {
	...
	context('reconcilePermissions function error handling', () => {
		it('It should return the defaultWindowOptions list if first parameter is empty and the second parameter is always true', () => {
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
		...
		it('It should return the value of the first parameter if the first parameter has a value and the second parameter is empty', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
	...
	context('reconcilePermissions function - value does not change when there is no key match or match is not more restrictive (permission true -> false)', () => {
		it('It should return the value of the first parameter if the second parameter has no category matches', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
			const requestedPermissions = createPermissionObject(['MessageBus', 'System'], ['publish', 'subscribe', 'clearCache', 'blur'], 'false');
		...
		it('It should return the value of the first parameter if the second parameter has category matches but no name matches', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['getWindows', 'getManifest', 'close'], 'false');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			expect(returned).to.deep.equal(currentPermissions);
		});
		...
		it('It should return the value of the first parameter if the second parameter has category and name matches but the value is always true', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
			const requestedPermissions = createPermissionObject(['WindowProcess'], ['close', 'createWindowProcess'], 'true');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			expect(returned).to.deep.equal(currentPermissions);
		});
		...
		it('It should return the value of the first parameter if the second parameter has category and name matches but first parameter contains only false', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'false');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'true');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			expect(returned).to.deep.equal(currentPermissions);
		});
		...
		it('It should return the value of the first parameter if the second parameter has category and name matches but the value is neither true nor false', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'true');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'junk');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			expect(returned).to.deep.equal(currentPermissions);
		});
		...
		it('It should return the value of the first parameter if the second parameter matches are only false when the first parameter is false and true when first parameter is true or false', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows', 'getOptions',  'blur', 'close', 'bringToFront'], 'alternate_true');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'alternate_true');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			expect(returned).to.deep.equal(currentPermissions);
		});
	});
	context('reconcilePermissions function - overwrite values that match and are more restrictive (permission true -> false)', () => {
		it('It should return the combination of first and second parameter where second parameter has matches and is more restrictive in all cases', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows', 'getOptions',  'blur', 'close', 'bringToFront'], 'true');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess',  'blur'], 'false');
			const finalBools = [false, false, true, true, false, true, true];
			const expectedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows', 'getOptions',  'blur', 'close', 'bringToFront'], 'specify', finalBools);
		...
		it('It should return the value of the second parameter when it has all keys and categories identical to the first parameter and every value is more restrictive in the second parameter', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'true');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'false');
			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
		...
		it('It should return the combination of first and second parameter with only some values getting overwritten when the second parameter is only more restrictive in some cases', () => {
			const currentPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'alternate_false');
			const requestedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'alternate_true');

			const returned = reconcilePermissions(currentPermissions, requestedPermissions);
			const expectedPermissions = createPermissionObject(['WindowProcess', 'Window'], ['close', 'createWindowProcess', 'getWindows',  'blur', 'close'], 'false');
			expect(returned).to.deep.equal(expectedPermissions);
		});
	});
